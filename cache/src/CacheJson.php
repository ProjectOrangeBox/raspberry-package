...
class CacheJson extends CacheAbstract implements CacheInterface
{
	protected $cachePath = '';
	...
	public function exists(string $name): bool
	{
		...
		return file_exists($this->cachePath . $name . '.json');
	}
	...
	public function save(string $name, $value, int $ttl = 60): bool
	{
		...
		return (bool)FS::atomic_file_put_contents($this->cachePath . $name . '.json', json_encode($content, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE));
	}
	...
	protected function getAllKeys(): array
	{
		...
		foreach ($files as $index => $file) {
			$files[$index] = substr($file, $strlen, -5); /* minus .json */
		}
