...
class CacheFile extends CacheAbstract implements CacheInterface
{
	...
	public function __construct(array $config)
	{
		...
		$this->cachePath = FS::resolve(rtrim($this->config['path'], '/')) . '/';
	...
	public function exists(string $name): bool
	{
		...
		return file_exists($this->cachePath . $name . '.cache');
	}
	...
	public function save(string $name, $value, int $ttl = 60): bool
	{
		...
		return (bool)FS::atomic_file_put_contents($this->cachePath . $name . '.cache', \serialize($content));
	}
	...
	protected function deleteSingle(string $name): bool
	{
		...
		$file = $this->cachePath . $name . '.cache';
	...
	protected function getAllKeys(): array
	{
		...
		foreach ($files as $index => $file) {
			$files[$index] = substr($file, $strlen, -6);
		}
	...
	protected function read(string $name)
	{
		...
		if ($this->exists($name)) {
			$data = \unserialize(\file_get_contents($this->cachePath . $name . '.cache'));
		}
	...
	protected function rsearch(string $folder): array
	{
		...
		$files = new \RegexIterator($ite, '/^.+\.cache$/i', \RegexIterator::GET_MATCH);
